...
namespace WpfSevens
{
    public class Table
    {
        ...
        public ICard Trun(ref string message)
        {
            ...
            var possibleCards = Table.GetPutPossibleCards(_PlayerCard[player], _PutCardList);
            message = "";
            ...
            if (card == null)
            {
                ...
                else
                {
                    if (possibleCards.Count > 0)
                        message = string.Format("{0}さんは、戦略的パスです。{1}回目です。", player.GetPalyerName(), _PlayerPassCount[player]);
                    else
